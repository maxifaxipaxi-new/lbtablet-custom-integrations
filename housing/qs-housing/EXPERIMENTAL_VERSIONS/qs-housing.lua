...
if Config.Framework == "esx" then
    ...
    searchPropertiesQuery = selectPropertyQuery .. [[
        WHERE
            ...
            OR p.id LIKE ?  -- Hier ebenfalls sicherstellen
...
elseif Config.Framework == "qb" then
    ...
    searchPropertiesQuery = selectPropertyQuery .. [[
        WHERE
            ...
            OR p.id LIKE ?  -- Hier auch sicherstellen
...
local function FormatPropery(property)
    ...
    if propertyData and #propertyData > 0 then
        local houseData = propertyData[1]
        property.label = houseData.label or property.id
        property.encodedId = EncodePropertyId(property.owner, property.id)  -- Store encoded ID separately
        ...
        property.name = nil
        -- property.id = nil -- REMOVE THIS LINE, so `id` stays

        if houseData.coords then
            property.location = houseData.coords or {}
        end
    end
...
function SearchProperties(query, page)
    ...
    print("Debug: Gefundene Properties ->", json.encode(properties))  -- Zeigt alle zurückgegebenen Daten an

    for i = 1, #properties do
        if not properties[i].id then
            print("Fehler: Property ID ist nil für Eintrag", json.encode(properties[i]))  -- Falls ID fehlt
        end
